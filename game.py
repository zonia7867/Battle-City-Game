...
# Phase 3 imports
from enemy_tank import SpawnManager
...
class MapGenerator:
    ...
    def get_level_weights(self, level: int):
        if level == 1:
            return {EMPTY: 55, BRICK: 32, STEEL: 5, WATER: 3, FOREST: 5}
        if level == 2:
            return {EMPTY: 50, BRICK: 20, STEEL: 15, WATER: 10, FOREST: 5}
        return {EMPTY: 55, BRICK: 25, STEEL: 10, WATER: 5, FOREST: 5}
    ...
    def assign_border_and_fixed(self):
        ...
        for px2, py2 in self.eagle_ring:
            self.map[py2][px2] = self.rng.choice([BRICK, STEEL])
    ...
    def check_forward(self):
        return self.can_reach_eagle() and self.player_reachable() and self.water_safety()
...
class Bullet:
    ...
    def advance_step(self, game_map, tanks, bullets, spawn_manager=None):
        if not self.alive:
            return
        ...
        if tile == BRICK:
            destroyed_xy = (self.x, self.y)
            game_map[self.y][self.x] = EMPTY
            self.alive = False
            # Notify BFS agents about destroyed brick
            if spawn_manager:
                spawn_manager.notify_map_changed(game_map, destroyed_xy)
            return
...
class Game:
    def __init__(self):
        ...
        pygame.display.set_caption('Battle City — Phase 3')
    ...
    def reset(self):
        ...
        self.bullets = []   # player bullets only

        # ── Phase 3: SpawnManager replaces static enemy list ──
        self.spawn_manager = SpawnManager(level=1)
    ...
    def reload_map(self):
        """R key — regenerate map and reset everything."""
        self.game_map = self.map_generator.generate(1)
        self.bullets  = []
        self.spawn_manager = SpawnManager(level=1)
        self.game_over   = False
    ...
    def handle_player_input(self):
        if not self.player.alive:
            return
        # Build all_tanks for collision (player + active enemies)
        all_tanks = self._all_tanks()
        if self.pressed[pygame.K_UP]:
            self.player.move('UP',    self.game_map, all_tanks)
        elif self.pressed[pygame.K_DOWN]:
            self.player.move('DOWN',  self.game_map, all_tanks)
        elif self.pressed[pygame.K_LEFT]:
            self.player.move('LEFT',  self.game_map, all_tanks)
        elif self.pressed[pygame.K_RIGHT]:
            self.player.move('RIGHT', self.game_map, all_tanks)
        ...
        if self.player.can_shoot:
            ...
            if not any(b.alive and b.owner is self.player for b in self.bullets):
                ...
                if 0 <= bx < GRID_WIDTH and 0 <= by < GRID_HEIGHT:
                    self.bullets.append(Bullet(bx, by, self.player.direction, self.player))
            self.player.can_shoot = False

    def advance_player_bullets(self):
        all_tanks = self._all_tanks()
        for bullet in self.bullets:
            if bullet.alive:
                # 2 steps per tick for bullet speed
                bullet.advance_step(self.game_map, all_tanks, self.bullets, self.spawn_manager)
                if bullet.alive:
                    bullet.advance_step(self.game_map, all_tanks, self.bullets, self.spawn_manager)
        # Eagle hit by player bullet
        for bullet in self.bullets:
            if not bullet.alive:
                continue
            if self.game_map[bullet.y][bullet.x] == EAGLE:
                self.game_over   = True
                self.lose_reason = 'Eagle destroyed!'
                bullet.alive = False
        self.bullets = [b for b in self.bullets if b.alive]

    # ── Enemy update (Phase 3) ───────────────────────────────────
    def update_enemies(self):
        px, py    = self.player.x, self.player.y
        all_tanks = self._all_tanks()

        self.spawn_manager.update(self.game_map, px, py, all_tanks)

        # Check if eagle was hit by an enemy bullet
        if self.spawn_manager.eagle_hit:
            self.game_over   = True
            self.lose_reason = 'Eagle destroyed by enemy!'
    ...
    def check_win_lose(self):
        if self.spawn_manager.all_defeated():
            self.game_over = True
            self.win       = True

    # ── Helpers ──────────────────────────────────────────────────
    def _all_tanks(self):
        """Return player + all active enemy tanks as a flat list."""
        return [self.player] + self.spawn_manager.active
    ...
    def render(self):
        ...
        self.draw_map()
        self.draw_player()
        self.spawn_manager.draw(self.screen)   # draws enemies + enemy bullets
    ...
    def draw_player(self):
        ...
        pygame.draw.rect(self.screen, PLAYER_DIR_COLOR, self.player.direction_rect())
    ...
    def draw_hud(self):
        ...
        remaining = self.spawn_manager.remaining_count()
        killed    = self.spawn_manager.total_killed
        active    = len(self.spawn_manager.active)
        ...
        for line in lines:
            ...
            self.screen.blit(surf, (GAME_WIDTH + 10, y))
            y += 24
    ...
    def draw_end_screen(self):
        ...
        sub     = self.lose_reason or 'Press R to restart.'

        msg_surf = self.big_font.render(message, True, (255, 255, 255))
        sub_surf = self.font.render(sub, True, (200, 200, 200))

        rect1 = msg_surf.get_rect(center=(GAME_WIDTH // 2, GAME_HEIGHT // 2 - 20))
        rect2 = sub_surf.get_rect(center=(GAME_WIDTH // 2, GAME_HEIGHT // 2 + 30))
        self.screen.blit(msg_surf, rect1)
